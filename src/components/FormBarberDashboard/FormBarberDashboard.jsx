...
const FormBarberDashboard = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.nome || !formData.whatsapp || !formData.data || !formData.horario || !formData.servico) {
    ...
    <div className="app-container">
      <div className="form-card">
        <div className="form-header">
          <button onClick={() => navigate(-1)} className="btn-home" title="Voltar"><Home size={20} /></button>
        ...
        {submitted ? (
          <div className="success-message">
            ...
            <p className="success-text">O agendamento foi registrado com sucesso! 💈</p>
          ...
          <div className="form-content">
            ...
            <div className="form-group">
              <label className="form-label"><User size={18} /><span>Nome do Cliente *</span></label>
              <input type="text" name="nome" placeholder="Ex: João Silva" value={formData.nome} onChange={handleInputChange} className="form-input" />
            </div>

            <div className="form-group">
              <label className="form-label"><Phone size={18} /><span>WhatsApp *</span></label>
              <input type="tel" name="whatsapp" placeholder="(85) 99999-9999" value={formData.whatsapp} onChange={handleInputChange} maxLength="15" className="form-input" />
            </div>

            <div className="section-title">📅 Dados do Agendamento</div>
            <div className="form-group">
              <label className="form-label"><Calendar size={18} /><span>Data *</span></label>
              <input type="date" name="data" value={formData.data} onChange={handleInputChange} min={hoje} max={diaMaxString} className={`form-input ${diaFechado ? "input-error" : ""}`} />
              {diaFechado && <p className="form-error">❌ Domingo e Segunda-feira: Estabelecimento fechado</p>}
            </div>

            <div className="form-group">
              <label className="form-label"><Clock size={18} /><span>Horário *</span></label>
              <select name="horario" value={formData.horario} onChange={handleInputChange} disabled={!formData.data || diaFechado || (formData.data && horariosDisponiveis.length === 0)} className="form-select">
                {!formData.data && <option value="">Selecione a data</option>}
                {formData.data && diaFechado && <option value="">Barbearia Fechada</option>}
            ...
            <div className="form-group">
              ...
              <select name="servico" value={formData.servico} onChange={handleInputChange} className="form-select">
            ...
            <div className="form-group">
              <label className="form-label"><span>Adicionais (Opcional)</span></label>
              <div className="adicionais-grid">
                {adicionais.map((ad) => (
                  <label key={ad.id} className={`adicional-item ${formData.adicionais.includes(ad.id) ? "adicional-selected" : ""}`}>
                    <input type="checkbox" checked={formData.adicionais.includes(ad.id)} onChange={() => handleAdicionaisChange(ad.id)} className="adicional-checkbox" />
                    <span className="adicional-emoji">{ad.emoji}</span>
                    <span className="adicional-text">{ad.nome}<small>+R$ {ad.preco.toFixed(2)}</small></span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="resumo-card">
              <div className="resumo-item"><span className="resumo-label">Cliente:</span><span className="resumo-value">{formData.nome || "Não informado"}</span></div>
              <div className="resumo-item"><span className="resumo-label">WhatsApp:</span><span className="resumo-value">{formData.whatsapp || "Não informado"}</span></div>
              <div className="resumo-item"><span className="resumo-label">Serviço:</span><span className="resumo-value">{servicoAtual?.nome || "Não selecionado"}</span></div>
              {formData.data && <div className="resumo-item"><span className="resumo-label">Data:</span><span className="resumo-value">{new Date(formData.data).toLocaleDateString("pt-BR")}</span></div>}
              {formData.horario && <div className="resumo-item"><span className="resumo-label">Horário:</span><span className="resumo-value">{formData.horario}</span></div>}
            </div>
            ...
            <button onClick={handleSubmit} className="submit-button" disabled={loading || loadingServicos}>
              {loading ? <Loader size={18} className="spin" /> : <><Scissors size={18} /> Criar Agendamento</>}
            </button>
          </div>
        )}
