...
const RedefinirSenha = () => {
    ...
    useEffect(() => {
        if (!token) {
            setStatusMessage({ 
                type: 'error', 
                message: 'Link inválido. O token de redefinição está ausente. Por favor, solicite a redefinição novamente.'
            });
        }
    }, [token]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (newPassword !== confirmPassword) {
            setStatusMessage({ type: 'error', message: 'As senhas não coincidem. Tente novamente.' });
            return;
        }

        if (newPassword.length < 8) {
            setStatusMessage({ type: 'error', message: 'A nova senha deve ter no mínimo 8 caracteres.' });
            return;
        }
        
        if (!token) {
            setStatusMessage({ type: 'error', message: 'Token de redefinição ausente. Não é possível continuar.' });
            return;
        }
        ...
        try {
            ...
            const errorMessage = error.error || 'Erro ao redefinir a senha. Tente novamente ou solicite um novo link.';
